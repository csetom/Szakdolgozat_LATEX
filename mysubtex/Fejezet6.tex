...
Példának okáért, tegyük fel, hogy nem transformálom át a gyűjtött adatokat, és a játék $N$ lépésből áll. 
...
Természetesen fennállt annak a lehetősége, hogy a betanított modellem nem lett teljesen pontos, azonban ez a kockázat minden transzformációs folyamat során előfordulhat.
...
\begin{figure}[h]
    ...
    \caption{Kód részlet, amely leképezi a $key$ által megadott játék lépéseit egy hash tömbbé. A hash-ek mellett a következő lépéseket tartalmazó tömbbel tér vissza.}
...
A betanított modellt egy $my_model.keras$ fájlba mentettem le, így könnyen tudtam használni, a játék futtatásakor. 
...
A Flask használatával készítettem egy webalkalmazást (\ref{code:use_model}. ábra), melynek egyetlen belépési pontja van, a $\/predict$. A Memoria játék ide küldi a $POST$ HTTP kérését, mely tartalmazza a játék aktuális állását. 
